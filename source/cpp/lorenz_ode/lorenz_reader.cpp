...
void read_solution()
{
    ...
    {
        std::cerr
            << "lorenz.bp doesn't exist; have you run ./bin/lorenz_writer?\n";
        return;
    }
    ...
    auto interpretation_att =
        io.InquireAttribute<std::string>("interpretation");
    ...
    for (auto const &[key, val] : io.AvailableVariables())
    {
        ...
        std::cout << "First position is u(0) = {" << u[0] << ", " << u[1]
                  << ", " << u[2] << "}\n";
        u = solution(solution.tmax());
        std::cout << "Last position is u(" << solution.tmax() << ") = {" << u[0]
                  << ", " << u[1] << ", " << u[2] << "}\n\n\n";
...
int main()
{
    ...
    {
        std::cerr << "Caught exception reading Lorenz ODE solution: "
                  << e.what() << "\n";
        return 1;
    }
    return 0;
