...
public:
           ...
           const Real absolute_error_goal)
    {
        ...
        while (t < tmax)
        {
            ...
            states_.emplace_back(
                std::array<Real, 7>{t, x, y, z, dotx, doty, dotz});
        }
    }
        ...
        for (size_t i = 1; i < states_.size(); ++i)
        {
            ...
            if (ti <= t)
            {
                throw std::logic_error(
                    "Deserialization is incorrect: Times are not sorted in "
    ...
    std::array<Real, 3> operator()(Real t) const
    {
        ...
        auto it =
            std::upper_bound(states_.begin(), states_.end(), t, comparator);
...
void test_lorenz()
{
    ...
    auto solution = lorenz<double>(sigma, beta, rho, initial_conditions, tmax,
                                   absolute_error);
