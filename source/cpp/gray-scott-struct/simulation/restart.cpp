               ...
               const GrayScott &sim, adios2::IO io)
{
    ...
    std::cout << "checkpoint at step " << step << " create file "
              << settings.checkpoint_output << std::endl;
    ...
    if (writer)
    {
        ...
        if (firstCkpt)
        {
            ...
            var_uv = io.DefineVariable<DataType>("UV", {N, X, Y, Z},
                                                 {R, 0, 0, 0}, {1, X, Y, Z});
        ...
        const DataType *ptr =
            reinterpret_cast<const DataType *>(sim.d_ghost().data());
