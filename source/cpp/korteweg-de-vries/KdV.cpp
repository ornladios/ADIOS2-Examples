 ...
 * accompanying file Copyright.txt for details. Solves the initial value problem
 * for the Korteweg de-Vries equation via the Zabusky and Krustal scheme: See:
 * https://journals.aps.org/prl/pdf/10.1103/PhysRevLett.15.240 Zabusky, Norman
 * J., and Martin D. Kruskal. "Interaction of solitons in a collisionless plasma
 * and the recurrence of initial states." Physical review letters 15.6 (1965):
 * 240.
 */
#include <adios2.h>
#include <cmath>
...
void KdV(int64_t N, Real dt, Real t_max, Real delta = 0.022)
{
    ...
    std::cout << "Using ∆x = " << dx << ", ∆t = " << dt
              << " and t_max = " << t_max << "\n";
    adios2::ADIOS adios;
    adios2::IO io = adios.DeclareIO("myio");
    auto u_variable = io.DefineVariable<Real>(
        "u", {size_t(N)}, {0}, {size_t(N)}, adios2::ConstantDims);
    ...
    adios2::Engine adios_engine =
        io.Open("korteweg_de_vries.bp", adios2::Mode::Write);
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1)
    {
        ...
        {
            std::cout << "Usage: ./KdV N t_max δ, where N is number of spacial "
                         "gridpoints (∆t chosen from ∆x via the stability "
                         "condition), t_max is max simulation time, and δ is "
                         "an interaction parameter; e.g., ./KdV 512 10 0.022\n";
            return 0;
        }
        N = std::stoi(dx_str);
    }
